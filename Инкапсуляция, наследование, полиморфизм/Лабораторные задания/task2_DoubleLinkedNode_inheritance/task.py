...
class DoubleLinkedNode(Node):
    def __init__(self, value, next_, prev: Optional["Node"] = None):
        super().__init__(value, next_)
        self.prev = prev  # TODO реализовать класс DoubleLinkedNode

    def __repr__(self):
        next_repr: str = str(None) if self.next is None else f"DoubleLinkedNode({self.next.value}, {None}, {None})"
        prev_repr: str = str(None) if self.prev is None else f"DoubleLinkedNode({self.next.value}, {None}, {None})"
